...
public class PassportWriter<T extends Passport>
{
    ...
    {
        {
            put(byte.class, "ValueLayout.JAVA_BYTE");
        ...
        for (Class<?> c : records)
        {
            ...
            for (Field f : c.getDeclaredFields())
            {
                ...
                else if (type.isRecord())
                {
                    ...
                    if (isPtr)
                        sbLayout.append(String.format("\t\tValueLayout.ADDRESS.withName(\"%s\"),\n", f.getName()));
                    else
    ...
    public String buildStoreStructFunction(Set<Class<?>> records)
    {
        ...
        for (Class<?> c : records)
        {
            ...
            for (Field f : c.getDeclaredFields())
            {
                ...
                if (type.isPrimitive())
                    sb.append(String.format("\t\tmemStruct.set(ValueLayout.JAVA_%2$s, %3$s, rec.%1$s());\n", f.getName(), typeToName.get(type).toUpperCase(), offset));
                else if (type.isRecord())
                {
                    boolean isPtr = f.getAnnotationsByType(Ptr.class).length > 0;
                    if (isPtr)
                        sb.append(String.format("\t\tmemStruct.set(ValueLayout.ADDRESS, %3$s, store%2$s(scope, rec.%1$s()));\n", f.getName(), type.getSimpleName(), offset));
                    else
                        sb.append(String.format("\t\tmemStruct.asSlice(%3$s).copyFrom(store%2$s(scope, rec.%1$s()));\n", f.getName(), type.getSimpleName(), offset));
                }
                else if (String.class.equals(type))
                    sb.append(String.format("\t\tmemStruct.set(ValueLayout.ADDRESS, %2$s, Utils.toCString(rec.%1$s(), scope).address());\n", f.getName(), offset));
                else if (type.isArray())
                {
                    Class<?> arrType = type.getComponentType();
                    Annotation[] arrays = f.getAnnotationsByType(Array.class);
                    ...
                    if (arrType.isPrimitive())
                    {
                        if (arrays.length > 0)
                            ...
                            sb.append(String.format("\t\tmemStruct.set(ValueLayout.ADDRESS, %2$s, Utils.toMS(scope, rec.%1$s()).address());\n", f.getName(), offset));
                    }
    ...
    private String buildReadStructFunction(Set<Class<?>> records)
    {
        ...
        for (Class<?> c : records)
        {
            ...
            for (Field f : c.getDeclaredFields())
            {
                ...
                Class<?> type = f.getType();
                if (type.isPrimitive())
                    sb.append(String.format("\t\tvar %1$s = memStruct.get(ValueLayout.JAVA_%2$s, %3$s);\n", f.getName(), typeToName.get(type).toUpperCase(), offset));
                else if (type.isRecord())
                {
                    boolean isPointer = f.getAnnotationsByType(Ptr.class).length > 0;
                    if (isPointer)
                        sb.append(String.format("\t\tvar %1$s = read%2$s(Utils.slice(memStruct.scope(), memStruct.get(ValueLayout.ADDRESS, %3$s), %2$sLayout.byteSize()), rec.%1$s());\n", f.getName(), type.getSimpleName(), offset));
//                        sb.append(String.format("\t\tvar %1$s = read%2$s(getAddressAtOffset(memStruct, %3$s).asSegment(%2$sLayout.byteSize(), memStruct.scope()), rec.%1$s());\n", f.getName(), type.getSimpleName(), offset));
                    ...
                    sb.append(String.format("\t\tvar %1$s = Utils.readString(memStruct.get(ValueLayout.ADDRESS, %2$s));\n", f.getName(), offset));
                else if (type.isArray())
                {
                    Class<?> arrType = type.getComponentType();
                    Annotation[] arrays = f.getAnnotationsByType(Array.class);
                    ...
                    if (arrType.isPrimitive())
                    {
                        if (arrays.length > 0) {
                            int length = ((Array) arrays[0]).length();
                            sb.append(String.format("\t\tvar %1$s = memStruct.asSlice(%4$s, %2$d * %3$s.BYTES).toArray(ValueLayout.JAVA_%5$s);\n", f.getName(), length, typeToName.get(arrType), offset, typeToName.get(arrType).toUpperCase()));
                        }
                        else if (isPointer)
                        {
                            sb.append(String.format("\t\tint %1$sSize = rec.%1$s().length;\n", f.getName()));
                            sb.append(String.format("\t\tvar %1$s = Utils.toArr%2$s(memStruct.get(ValueLayout.ADDRESS, %3$s), %1$sSize);\n", f.getName(), typeToName.get(arrType), offset));
                        }
                    }
                }
            }
    ...
    public void addMethod(Method method, Class<?> retType)
    {
        ...
        for (Class<?> parameter : method.getParameterTypes())
        {
            ...
            if (isArrayOfPrimitives(parameter) || is2DArrayOfPrimitives(parameter))
            {
                ...
                params.append("(Addressable)Utils.toAddr(vv").append(v).append("),");
            ...
            {
                bHasAllocatedMemory = true;
                preCall.append(String.format("var vv%1$d = v%1$d == null ? MemoryAddress.NULL : Utils.toCString(v%1$d, scope);\n", v));
                params.append("(Addressable)vv").append(v).append(',');
            }
        ...
        if (bHasAllocatedMemory)
        {
            ...
            preCall.insert(0, "var allocator = SegmentAllocator.nativeAllocator(scope);\n\t\t");
        }
    ...
    private static Set<Class<?>> findAllExtraImports(List<Method> interfaceMethods) {
        ...
        for (Method m : interfaceMethods) {
            ...
            if (!isValidArgType(retType))
                throw new PassportException("Types in the interface must be primitive, arrays of primitives, String, or Records. " + retType.getSimpleName() + " not supported.");

            List<Class<?>> invalid = Arrays.stream(params).filter(p -> !isValidArgType(p)).toList();
            if (!invalid.isEmpty())
                throw new PassportException("Types in the interface must be primitive, arrays of primitives, String, or Records. " + invalid.get(0).getSimpleName() + " not supported.");
    ...
    private static boolean isValidArgType(Class<?> c)
    {
        ...
        if (c.isArray() && (c.componentType().isPrimitive() || c.getComponentType().isRecord()))
            return true;
        if (Addressable.class.equals(c) || MemoryAddress.class.equals(c) || String.class.equals(c))
            return true;
