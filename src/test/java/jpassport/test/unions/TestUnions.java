...
import jpassport.test.structs.TestStructCalls;
import jpassport.test.structs.TestUsingStructs;
...
import static jpassport.Union.NO_TO_NATIVE;
...
public class TestUnions {
    ...
    public record SimpleUnion (
            ...
            float u_f,
            double u_d,
            @UnionToNativeIdx  int toNative,
            @UnionFromNativeIdx int fromNative
    ) implements Union {}
    ...
    public void testPrimitivesUnion()
    {
        ...
        {
            SimpleUnion[] suArr  = new SimpleUnion[] {new SimpleUnion((short)7, 8, 0, 0, 0,0, 4)};
    ...
    void testUnionWithStruct()
    {
        for (Link link : PassingUnions) {
            ...
            var uws = new UnionWithStruct[] {new UnionWithStruct(0, new PassingData(0,0,0,0), (short)0, NO_TO_NATIVE, 1)};
    ...
    void testUnionWithArrays()
    {
        for (Link link : PassingUnions)
        {
            var wa = new UnionWithArrays(new long[] {1,2,3,4,5}, new long[5],0, 1);
            var uwa = new UnionWithArrays[] {wa};
            link.link.useUnionWithArray(1, uwa);
            assertArrayEquals(wa.u_i, uwa[0].u_ip);

            wa = new UnionWithArrays(new long[5], new long[] {10,11,12,13,14},1, 0);
